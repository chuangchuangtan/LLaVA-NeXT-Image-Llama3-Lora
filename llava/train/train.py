...
def find_all_linear_names(model):
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, cls):
            ...
            lora_module_names.add(name)  # split 之后又会appen 内的 multimodal_keywords. 例如 q_proj 在 vision_tower 也含有。
...
def preprocess_llama_3(
    ...
    # Mask targets
    sep = conv.sep + '<|start_header_id|>' + conv.roles[1] + '<|end_header_id|>\n\n'   # 用于分开que1与ans1, split后最后一个为''
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for conv_idx in range(3, len(rounds), 2):
            re_rounds.append(conv.sep.join(rounds[conv_idx:conv_idx + 2])) # re_rounds: [system-que1-ans1, que2-ans2, ...]
        ...
        for i, rou in enumerate(re_rounds):
            ...
            if has_image:
                round_len = len(tokenizer_image_token(rou, tokenizer))# + 1        # 整个问答的长度
                instruction_len = len(tokenizer_image_token(parts[0], tokenizer)) # que 的长度
            else:
            ...
            target[cur_len: cur_len + instruction_len] = IGNORE_INDEX             # 将问题部分置-100
            cur_len += round_len
        cur_len += 1
        target[cur_len:] = IGNORE_INDEX
...
def train(attn_implementation=None):
    ...
    if model_args.version == "v0":
        ...
        if model_args.version in conversation_lib.conv_templates:
            ...
            conversation_lib.default_conversation = conversation_lib.conv_templates["llama_v3"]
    ...
    for name, pa in model.named_parameters():
        if pa.requires_grad: print(name, pa.requires_grad)
